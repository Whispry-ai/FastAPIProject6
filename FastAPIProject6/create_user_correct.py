#!/usr/bin/env python3
"""
Create user with correct column count (12 columns, no id)
"""
...
def create_user():
    """Create user with correct 12 columns"""
    print("👤 Creating user...")
    ...
    try:
        ...
        # Create user with 12 columns (excluding auto-increment id)
        user_uid = str(uuid.uuid4())
        cursor.execute("""
            INSERT OR REPLACE INTO users 
            (user_uid, email, phone, name, role, created_at, updated_at, is_active, email_verified, mobile_verified, token_version)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (user_uid, "kamineniaswini@gmail.com", "+7601002908", "Admin User", 5, datetime.now(), datetime.now(), 1, 1, 1, 0))
...
def main():
    print("🚀 Create User (Correct)")
    print("=" * 50)
    
    if create_user():
        print("\n" + "=" * 50)
        print("✅ User created! Login with:")
