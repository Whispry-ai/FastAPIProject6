...
def create_user():
    """Create user with minimal required columns"""
    print("👤 Creating user...")
    ...
    try:
        ...
        # Create user with only 7 columns
        user_uid = str(uuid.uuid4())
        cursor.execute("""
            INSERT OR REPLACE INTO users 
            (user_uid, email, phone, name, role, created_at, is_active, email_verified)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (user_uid, "kamineniaswini@gmail.com", "+7601002908", "Admin User", 5, datetime.now(), 1, 1))
